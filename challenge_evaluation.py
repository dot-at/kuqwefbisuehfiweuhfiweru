...
GLOBAL_TIMEOUT = 3600
ERROR_LIMIT = 1e-10
from qiskit import QuantumProgram, qasm, unroll
...
def evaluate(compiler_function=None, test_circuits=None, verbose=False, backend = 'local_qiskit_simulator'):
    ...
    if compiler_function is not None:
        # step through all the test circuits
        compile_jobs = [[name,circuit,verbose,compiler_function,gate_costs] for name, circuit in test_circuits.items()]
        ...
        for job in range(len(compile_jobs)):
            ...
            test_circuits[name] = res_values[job].pop("circuit") # remove the circuit from the results and store it
            results[name] = res_values[job] # store the other values in the result dict
        ...
        results[name]["sim_time_orig"] = time.perf_counter() - time_start
        ...
        results[name]["sim_time_opti"] = time.perf_counter() - time_start
        ...
        if verbose:
            ...
            # results[name]["sim_time_ref"] = time.perf_counter() - time_start
...
def _compile_circuits(compile_args):
    ...
    try:
        ...
        res_values["optimizer_time"] = time.perf_counter() - time_start
    except:
        ...
        print(
        "An error occurred in the user compiler while running " + name + " on " + str(circuit["coupling_map"]) + '.')
        pass
    if verbose:
        ...
        try:
            ...
            circuit["dag_reference"] = _qiskit_compiler(
                circuit["dag_original"], coupling_map=circuit["coupling_map"],
                gate_costs=gate_costs)
            res_values["reference_time"] = time.perf_counter() - time_start
        except:
    ...
    res_values["circuit"] = circuit  # does this need a deep copy?
...
def _qiskit_compiler(dag_circuit, coupling_map=None, gate_costs='default'):
    ...
    compiled_dag, final_layout = swap_mapper(copy.deepcopy(dag_circuit), coupling, initial_layout, trials=20)
    # Expand swaps
...
def _compare_outputs(data_original, data_compiled, permutation, threshold=ERROR_LIMIT):
    ...
    fidelity = abs(target.conj().dot(state)) ** 2
